...
(function() {
    ...
    const updateMessage = "Added support to load a Google MyMap data into WME.  This is a one-way load - it is not possible to delete the MyMap pins from WME.  Don't ask.<br><br>Scroll to the bottom of the OOM tab and paste a Google MyMap URL into the box and press Load MyMap.  The MyMap must be publicly accessible.";
...
style.graphicOpacity=1;style.externalGraphic=this.iconColorMap[color.color];style.graphicYOffset=-32;}}
            var hotSpotNode=this.getElementsByTagNameNS(styleTypeNode,"*","hotSpot")[0];if(hotSpotNode){var x=parseFloat(hotSpotNode.getAttribute("x"));var y=parseFloat(hotSpotNode.getAttribute("y"));var xUnits=hotSpotNode.getAttribute("xunits");if(xUnits=="pixels")
                style.graphicXOffset=-x*scale;else if(xUnits=="insetPixels")
                    style.graphicXOffset=-width+(x*scale);else if(xUnits=="fraction")
                        style.graphicXOffset=-width*x;var yUnits=hotSpotNode.getAttribute("yunits");if(yUnits=="pixels")
                            style.graphicYOffset=-height+(y*scale)+1;else if(yUnits=="insetPixels")
                                style.graphicYOffset=-(y*scale)+1;else if(yUnits=="fraction")
